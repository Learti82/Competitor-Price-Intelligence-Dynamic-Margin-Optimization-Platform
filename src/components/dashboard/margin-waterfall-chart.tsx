...
const CustomTooltip = ({
  ...
  active?: boolean;
  payload?: Array<{ payload: WaterfallStep & { visible: number } }>;
}) => {
    ...
    return (
      <div className="rounded-lg border border-gray-700 bg-gray-900 p-3 text-xs shadow-xl">
        ...
        <p style={{ color: step.color }}>
          {step.value < 0 ? "-" : ""}€
          {Math.abs(step.value).toLocaleString("sq-AL", {
...
export function MarginWaterfallChart({ steps }: MarginWaterfallChartProps) {
