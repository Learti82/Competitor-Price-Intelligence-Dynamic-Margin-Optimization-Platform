...
const DEMO_ORG = "demo_org_markal";

async function getCompany() {
  return db.company.findFirst({ where: { clerkOrgId: DEMO_ORG } });
}
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ type: string }> }) {
  try {
    ...
    if (type === "products") {
      ...
      sheetName = "Produktet";
      data = products.map((p) => ({
        SKU: p.sku, Barcode: p.barcode ?? "", Emri: p.name,
        Brendi: p.brand ?? "", Kategoria: p.category,
        "Çmimi Aktual (€)": p.currentPrice, "COGS (€)": p.cogs,
        "Marzhi (%)": p.currentMargin.toFixed(2),
        "Marzhi Min (%)": p.minMargin, "Marzhi Max (%)": p.maxMargin,
      ...
      data = competitors.map((c) => ({
        Emri: c.name, Qyteti: c.hqCity,
        "Numri i Dyqaneve": c.numberOfStores,
        Strategjia: c.pricingStrategy ?? "",
        Website: c.websiteUrl ?? "", Aktiv: c.isActive ? "Po" : "Jo",
      }));
      ...
      data = recs.map((r) => ({
        ...
        "Marzhi Aktual (%)": r.currentMargin.toFixed(2),
        "Marzhi i Rekomanduar (%)": r.recommendedMargin.toFixed(2),
        "Besueshmëria (%)": (r.confidenceScore * 100).toFixed(0),
        "Delta Të Ardhurat (€)": r.expectedRevenueDelta.toFixed(2),
        Statusi: r.status, Arsyeja: r.rationale,
        "Krijuar më": r.createdAt.toISOString().slice(0, 10),
      }));
    } else if (type === "alerts") {
